          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="lg:w-64">
              <Select value={selectedDepartment} onValueChange={setSelectedDepartment}>
                ...
                <SelectContent>{departments.map(dept => <SelectItem key={dept} value={dept}>{dept === 'all' ? 'All Departments' : dept}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="lg:w-48">
              <Select value={selectedSummary} onValueChange={setSelectedSummary}>
                <SelectTrigger><SelectValue placeholder="Summary Type" /></SelectTrigger>
                <SelectContent>
        ...
        <div className="grid gap-6">
          {publications.length === 0 ? (
            <div className="text-center py-12">
              ...
              <Button onClick={() => router.push('/profile')}>Add Your First Publication</Button>
            </div>
          ) : (
              ...
              <Card key={publication?.id} className="hover:shadow-md transition-shadow">
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <CardTitle className="text-xl mb-2 line-clamp-2">{publication?.title}</CardTitle>
                      <CardDescription className="flex items-center gap-4 text-sm">
                        <div className="flex items-center gap-1"><User className="h-4 w-4" /><span>{publication?.author?.name ?? 'Unknown'}</span></div>
                        <div className="flex items-center gap-1"><Calendar className="h-4 w-4" /><span>{formatDate(publication?.publicationDate)}</span></div>
                        <div className="flex items-center gap-1"><TrendingUp className="h-4 w-4" /><span>{publication?.citations ?? 0} citations</span></div>
                      </CardDescription>
                    </div>
                    <Badge variant="secondary" className="ml-4">{publication?.department ?? 'N/A'}</Badge>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex flex-wrap gap-2">
                      {publication?.authors ? publication.authors.split(',').map((author, index) => <Badge key={index} variant="outline" className="text-xs">{author.trim()}</Badge>) : null}
                    </div>
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <Brain className="h-4 w-4 text-blue-600" />
                        <span className="text-sm font-medium text-gray-700">{selectedSummary === 'hybrid' ? 'AI-Generated Summary' : selectedSummary === 'extractive' ? 'Extractive Summary' : 'Abstractive Summary'}</span>
                      </div>
                      <p className="text-gray-600 leading-relaxed">{getSummaryText(publication)}</p>
                    </div>
                    {publication?.keywords ? (<div className="flex flex-wrap gap-2">{publication.keywords.split(',').map((keyword, index) => <Badge key={index} variant="outline" className="text-xs bg-blue-50 text-blue-700 border-blue-200">{keyword.trim()}</Badge>)}</div>) : null}
                    <div className="flex items-center justify-between pt-4 border-t">
                      <div className="text-sm text-gray-500">Published in <span className="font-medium">{publication?.journal ?? '—'}</span></div>
                      <div className="flex items-center gap-2"><Link href={`/publications/${publication?.id}`}><Button variant="outline" size="sm"><Eye className="h-4 w-4 mr-2" />View Details</Button></Link></div>
                    </div>
                  </div>
                </CardContent>
              </Card>
