...
export async function GET(request: NextRequest) {
  try {
    ...
    const publicationTrends = publications.reduce((acc: any[], pub) => {
      ...
      if (existing) {
        ...
        acc.push({ year, count: 1, citations: pub.citations })
      }
      return acc
    }, []).sort((a, b) => a.year - b.year)

    // 2. Department statistics
    const departmentStats = faculty.reduce((acc: any[], member) => {
    ...
    const researchAreas = faculty
      .filter(member => member.specialization)
