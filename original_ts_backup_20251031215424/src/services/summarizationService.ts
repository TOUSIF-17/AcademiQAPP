...
type SentenceScore = { index: number; score: number };
...
class SummarizationService {
  ...
  private wordSplit(text: string): string[] {
    return text
      .toLowerCase()
      .replace(/[^a-z0-9\s]/g, ' ')
      .split(/\s+/)
  ...
  private stopwords = new Set([
    'a','an','and','the','is','are','was','were','in','on','for','to','of','by','with','as','that','this','it','from','at','be','or','we','our','their','his','her','they','them','these','those','which','into','than','then','also','can','may','might','have','has','had','do','does','did','using','used','based','such','between','within','over','under','more','most','less','least','been','being','both','each','other','through','per','via'
  ]);
  ...
  private buildSimilarityMatrix(sentences: string[]): number[][] {
    ...
    const tf: number[][] = vectors.map(v => terms.map(t => (v[t] || 0)));
  ...
  private pageRank(sim: number[][], d = 0.85, maxIter = 40, eps = 1e-4): number[] {
    const n = sim.length;
    if (n === 0) return [];
  ...
  private buildSummary(sentences: string[], scores: number[], maxWords = 150): string {
    const ranked: SentenceScore[] = sentences.map((_, i) => ({ index: i, score: scores[i] || 0 }));
    ranked.sort((a, b) => b.score - a.score);
    const chosen: SentenceScore[] = [];
    let wordCount = 0;
